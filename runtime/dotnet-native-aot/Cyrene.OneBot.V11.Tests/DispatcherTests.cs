...
// │  Role: Generic DirectPluginRuntime validation tests.                     │
// │                                                                         │
// │  模块职责：通用 DirectPluginRuntime 校验测试                               │
// └─────────────────────────────────────────────────────────────────────────┘
...
public sealed class DispatcherTests
{
    ...
    {
        DirectInvocationRequest request = CreateRequest("message.connector.v1");
        request.Payload = ByteString.CopyFrom(
            new byte[NotConfiguredInvocationDispatcher.MaxPayloadBytes + 1]);
