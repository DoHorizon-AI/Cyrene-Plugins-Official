...
┌─────────────────────────────────────────────────────────────────────┐
│  📄 logging.py                                                      │
│  Module: cyrene_plugin_runtime.logging                              │
│  Role: Structured NDJSON logging and correlation sanitization.      │
│                                                                     │
│  模块职责：插件运行时结构化日志、上下文关联与不可信输入清洗。              │
│  严格要求：诊断信息与错误日志只允许输出到 stderr，绝不污染 stdout 机器协议。 │
└─────────────────────────────────────────────────────────────────────┘
"""
...
def format_cyrene_log(
    ...
    message: str,
    *,
    service_name: str = "cyrene-plugin-runtime",
...
def emit_diagnostic_error(
    ...
    *,
    ...
    """Emit a structured diagnostic error record directly to sys.stderr (NEVER stdout).

        中文：将结构化诊断错误记录直接写入 sys.stderr（绝不写入 stdout）。"""
